...
execute as @a[tag=limitless.purple.user] at @s if score @s user.id = %limitless.purple.search user.id as @e[type=area_effect_cloud,tag=limitless.purple,scores={generic.math=1..}] positioned ^ ^1.25 ^1.50 run tp @s ~ ~ ~ ~ ~
