...
execute unless block ~ ~2 ~ #src:passable run scoreboard players set @s generic.raycast.step -1
execute if score @s generic.raycast.step matches -1 run return run tp @s ~ ~ ~

# recursive call
scoreboard players remove @s generic.raycast.step 1
execute if score @s generic.raycast.step matches 0.. positioned ~ ~0.5 ~ run function src:weapon/yamato/upper_slash/launch_step