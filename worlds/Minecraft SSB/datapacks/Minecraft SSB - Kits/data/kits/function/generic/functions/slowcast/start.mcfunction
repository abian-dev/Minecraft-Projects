...
scoreboard players remove @s kits.generic.raycast.dist 1
$execute if score @s kits.generic.raycast.dist matches -1..1 run return run function kits:generic/functions/slowcast/hit_end {"slowcaster":"$(slowcaster)","hitEnd":"$(hitEnd)"}

# run slowcast
$scoreboard players set @s kits.generic.raycast.step $(speed)
