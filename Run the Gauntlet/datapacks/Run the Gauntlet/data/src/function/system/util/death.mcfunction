# example call: function src:system/util/death {"tpPlayer":"0","onDeathPos":"function"}
...
execute if score %player.death.boolTp system.global matches 1 run function src:system/util/death_tp with storage system.player death

# onDeathPos
summon marker ~ ~ ~ {Tags:["system.player.death.lastDeathPos"]}
execute as @e[type=marker,tag=system.player.death.lastDeathPos,limit=1,sort=nearest] at @s run function src:system/util/death_pos with storage system.player death
