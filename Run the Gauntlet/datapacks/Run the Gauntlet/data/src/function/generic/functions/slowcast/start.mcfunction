...
# example call: function src:generic/functions/slowcast/start {"speed":"10","slowcaster":"genericSlowcaster","hitEnd":"function","hitBlock":"function","hitEntity":"function","vfx":"function"}
## Slowcast Start Function
# limit travel distance
scoreboard players remove @s generic.raycast.dist 1
$execute if score @s generic.raycast.dist matches -1..1 run return run function src:generic/functions/slowcast/hit_end {"slowcaster":"$(slowcaster)","hitEnd":"$(hitEnd)"}

# run slowcast
$scoreboard players set @s generic.raycast.step $(speed)
$function src:generic/functions/slowcast/step {"slowcaster":"$(slowcaster)","hitBlock":"$(hitBlock)","hitEntity":"$(hitEntity)","vfx":"$(vfx)"}
